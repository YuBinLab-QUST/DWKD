...
parser.add_argument('--reg_weight', default=1e-3, type=float, help='regularization weight')  # 权重衰减
...
parser.add_argument('--ce_alpha', default=0.9, type=float, help='ce_alpha')
parser.add_argument('--en_alpha', default=0., type=float, help='ce_alpha')
parser.add_argument('--temperature', default=3.0, type=float, help='KL loss temperature')
parser.add_argument('--index', default=[0], type=int, nargs='+', help='uncertainty mode')
parser.add_argument('--teacher', default="small", type=str, help='uncertainty mode')
parser.add_argument('--ende', default="en", type=str, help='encoder or decoder')
parser.add_argument('--kd_logits', action='store_true', help='use squa')
parser.add_argument('--dy_loss', action='store_true', help='use squa')
...
def main_worker(gpu, args):
    ...
    # small teacher model
    pretrained_dir = args.pretrainedT_dir
    model_name = args.pretrained_model_name
    pretrained_pth = os.path.join(pretrained_dir, model_name)
    teacher_model = UNet(spatial_dims=3,
                         in_channels=4,
                         out_channels=3,
    ...
    # region student
    student_model = UNet(spatial_dims=3,
                         in_channels=4,
                         out_channels=3,
                         channels=(16, 32),
                         strides=(2, 2),
    ...
    if args.resume_ckpt:
        ...
        val_acc_max = torch.load(pretrained_pth)["best_acc"]
    ...
    model.cuda(args.gpu)
    # print(type(teacher_model), teacher_model.channels)
    # print(type(student_model), student_model.channels)
    if args.distributed:
        torch.cuda.set_device(args.gpu)
